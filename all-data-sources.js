...
async function fetchDataSources(pageUrl) {
  ...
  try {
    ...
    console.log(`Fetching data sources for interfaceId: ${interfaceId}...`);
    while (hasMorePages) {
      const requestBody = {
        filter: {
      ...
      console.log(`Request Payload (Page ${currentPage + 1}):`, JSON.stringify(requestBody, null, 2));
      ...
      console.log(`Response (Page ${currentPage + 1}):`, JSON.stringify(data, null, 2));
    ...
    if (allDataSources.length > 0) {
      const fileName = `samples/sample_e_data_source_all_${interfaceId}.json`;
      await fs.writeFile(fileName, JSON.stringify(allDataSources, null, 2));
      console.log(`Saved ${allDataSources.length} total data sources for ${interfaceId} to ${fileName}`);
    } else {
      console.log('No data sources found for the given interfaceId.');
    }
